...
```{todo}
Setup
- Copy and paste `./src/ada_course/chapter_template`, and rename according to `CHAPTER_NAMES` in `src/ada_course/config.py`.
...
- If not yet done, append folder name to `CHAPTER_NAMES` in `src/ada_course/config.py`.
- Specify materials to include in the book in `MOST_RECENT_MATERIALS` in `src/ada_course/config.py`.
